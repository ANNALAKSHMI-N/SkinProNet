...
from model import buildACRNN
from preprocess import preprocess
from utils import Worker, CLASSES, CLASS_NAMES
...
class MainGUI(QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Skin Lesion Detection")
    ...
    def classify_runner(self):
        ...
        model = buildACRNN(x.shape[1:], len(CLASSES))
        model.load_weights("model/model.h5")
        prob = model.predict(x)
        pred = np.argmax(prob, axis=1)[0]
        cls = CLASSES[pred]
        self.class_ = "Classified As :: {0} ({1:.2f}%)".format(
            CLASS_NAMES[cls], prob[0][pred] * 100
        )
        self.cls = CLASSES[pred]
    ...
    def segment_runner(self):
        mdl = U2NetPP()
        mdl.load_weights("u2netpp/model.h5")
        mask = segment(self._input_image_path, mdl)
        ...
        self.seg_data = {"Mask": mask, "Segmented": segmented}

    def segment_finisher(self):
        for k in self.seg_data:
            cv2.imwrite("tmp.jpg", self.seg_data[k])
            self.add_image("tmp.jpg", k)
        os.remove("tmp.jpg")
        self.load_screen.close()
        self.ci_pb.setEnabled(True)
